...
import { Clock, CheckCircle, XCircle, ClipboardList, MapPin, Users } from 'lucide-react';

const STATUS_CONFIG = {
  PENDING: { badge: 'badge-orange', Icon: Clock, label: 'Pending Review' },
...
export default function RecruiterJobs() {
  ...
  const summaryStats = [
    { label: 'Total Jobs', value: data.total, Icon: ClipboardList, color: 'from-indigo-500 to-violet-600' },
    { label: 'Approved', value: approved, Icon: CheckCircle, color: 'from-teal-400 to-emerald-500' },
    { label: 'Pending', value: pending, Icon: Clock, color: 'from-orange-400 to-amber-500' },
  ];
        ...
        <>
          ...
          <ul className="space-y-4">
            {data.jobs.map((job, idx) => {
              const cfg = STATUS_CONFIG[job.status] || { badge: 'badge-blue', Icon: ClipboardList, label: job.status };
